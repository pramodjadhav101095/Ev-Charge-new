...
import com.evcharge.admin.config.SecurityConfig;
...
@WebMvcTest(controllers = AdminController.class, excludeFilters = @org.springframework.context.annotation.ComponentScan.Filter(type = FilterType.ASSIGNABLE_TYPE, classes = SecurityConfig.class))
...
class AdminControllerTest {
    ...
    void getUsers_returnsList() throws Exception {
        UserResponse u = UserResponse.builder().id(1L).username("alice").email("a@example.com").role("ROLE_USER")
                .build();
     ...
     * Verifies that the endpoint returns a 204 No Content status and calls the
     * service method.
     */
    @WithMockUser(roles = "ADMIN")
    @Test
     ...
     * Verifies that the endpoint returns a 200 OK status and the correct station
     * data.
     */
    @WithMockUser(roles = "ADMIN")
    @Test
    void getStations_returnsList() throws Exception {
        StationResponse s = StationResponse.builder().id(10L).name("Station A").location("Loc").status("PENDING")
     ...
     * Verifies that the endpoint returns a 200 OK status and the created station
     * data.
     */
    @WithMockUser(roles = "ADMIN")
    @Test
    void createStation_returnsCreatedStation() throws Exception {
        StationResponse input = StationResponse.builder().name("New Station").location("L1").status("PENDING")
                .latitude(0.0).longitude(0.0).build();
        StationResponse created = StationResponse.builder().id(99L).name("New Station").location("L1").status("PENDING")
                .latitude(0.0).longitude(0.0).build();
     ...
     * Verifies that the endpoint returns a 200 OK status and the correct session
     * data.
     */
    @WithMockUser(roles = "ADMIN")
    @Test
    void getActiveSessions_returnsList() throws Exception {
        SessionResponse s = SessionResponse.builder().sessionId("sess1").userId("u1").stationId("st1").status("ACTIVE")
                .startTime(LocalDateTime.now()).energyConsumed(5.0).build();
     ...
     * Verifies that the endpoint returns a 200 OK status and the correct report
     * data.
     */
    @WithMockUser(roles = "ADMIN")
    @Test
    void getBillingReport_returnsReport() throws Exception {
        ReportResponse r = ReportResponse.builder().reportId("R1").type("BILLING").data("details")
